...
const ChildComments = ({ writer, userData, comment, index, deleteReply }) => {
  return (
    <>
      <li key={index} className='childComment'>
      ...
      {comment.children &&
        comment.children.map((childComment, idx) => (
          <ChildComments userData={userData} comment={childComment} key={idx} deleteReply={deleteReply} />
        ))}
    </>
  );
};
