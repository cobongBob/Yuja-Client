...
const Wregister = () => {
  ...
  let Yhistory = useCallback((board_id) => history.push(`/Winwin`), [history]);
  // let Yhistory = useCallback((board_id) => history.push(`/WDetail/${board_id}`), [history]);
  ...
  const testCheking = useCallback(() => {
    ...
    const sendingData = {
      ...inputData,
      userId: userData.id,
      content: qData.replaceAll(`src="http://localhost:8888/files/temp/`, `src="http://localhost:8888/files/WinBoard/`), //업로드된 이미지들은 temp가 아닌 WinBoard에 저장된다.
      thumbnail: "썸네일테스트", //썸네일 서버쪽 만들어지면 변경 필
      boardAttachNames: currFileList.current,
    };
    insertWinBoard(sendingData)
      .then((res) => {
