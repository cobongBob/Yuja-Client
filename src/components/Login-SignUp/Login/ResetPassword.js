...
const ResetPassword = ({ history, location }) => {
  ...
  const [resetPasswordData, setResetPasswordData] = useState({
    username: location.username.username,
    password: '',
  });
  ...
  const checkPasswordValidate = useCallback(() => {
    passCheck.test(dataInput.pass) === false && dataInput.pass !== ""
  ...
  const checkPasswordCheckValidate = useCallback(() => {
    dataInput.pass !== resetPassCheckNum && resetPassCheckNum !== ""
      ? setResetPasswordCheckDesc("비밀번호를 확인해주세요.")
      :
      setResetPasswordCheckDesc("");
  }, [dataInput, resetPassCheckNum]);
  ...
  return(
    <div className='resetPasswordFrag'>
      ...
      <div className='resetPasswordTitleBox'>
        <Link className='resetPasswordTitle' to='/'>유자 비밀번호 찾기</Link>
      </div>
      ...
      <div className='resetPasswordContentBox'>
        <div className='overlay'>
          ...
          <div className='labelWrapper'>
            <label htmlFor='signUpPw'>새로운 비밀번호</label>
          </div>
          <input
            className='signUpPw'
            name='password'
          ...
          <div className='labelWrapper'>
            <label htmlFor='signUpPwCheck'>새로운 비밀번호 확인</label>
          </div>
          <input
